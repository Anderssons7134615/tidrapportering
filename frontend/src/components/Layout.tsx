...
function BrandMark({ compact = false, onDark = false }: { compact?: boolean; onDark?: boolean }) {
  return <div className={`flex h-10 shrink-0 items-center justify-center ${onDark ? compact ? 'w-24 px-0 text-white' : 'w-28 px-0 text-white' : `rounded-md border border-graphite-200 bg-white px-2 ${compact ? 'w-24' : 'w-28'}`}`}><img src="/anderssons-logo.svg" alt="Anderssons Isolering" className={`h-7 w-full object-contain ${onDark ? 'brightness-0 invert' : ''}`} /></div>;
}
